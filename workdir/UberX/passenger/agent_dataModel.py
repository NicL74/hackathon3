...
class TIME_AGENT (DataModel):
	...
	def __init__(self):
		super().__init__("time_datamodel",[self.ATTRIBUTE_TIMEZONE,self.ATTRIBUTE_ID, self.ATTRIBUTE_TWENTYFOUR],"Agent has the time.")
...
class DRIVER_AGENT (DataModel):
	...
	def __init__(self):
        	super().__init__("driver_datamodel",[self.ATTRIBUTE_AREA,self.ATTRIBUTE_ID, self.ATTRIBUTE_DRIVERNAME, self.ATTRIBUTE_AVAILABLE],"Driver is potentially available to pick up a passenger.")
...
class PASSENGER_TRIP (DataModel):
	...
	def __init__(self):
        	super().__init__("trip_datamodel",[self.ATTRIBUTE_PICKUP,self.ATTRIBUTE_ID, self.ATTRIBUTE_DESTINATION],"Passenger trip is defined.")
